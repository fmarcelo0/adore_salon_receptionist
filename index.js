...
app.post('/voice', (req, res) => {
  ...
  res.send(`<?xml version="1.0" encoding="UTF-8"?>
    <Response>
      <Say><prosody rate="110%">Hello, thank you for calling Adore Salon. How can I help you today?</prosody></Say>
      <Connect>
  ...
  dgConnection.on(LiveTranscriptionEvents.Transcript, async (data) => {
    ...
    try {
      ...
      await twilioClient.calls(callSid).update({
        twiml: `<Response><Say><prosody rate="110%">${reply}</prosody></Say><Connect><Stream url="wss://franco-first-commit.onrender.com/stream"/></Connect></Response>`
      })
    } catch (err) {
      console.error(err)
    } finally {
